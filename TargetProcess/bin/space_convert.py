...
class Convert:
    ...
    def convert(self):
        ...
        *_, tar_ruler= tar_space.ruler()
        ...
        up_tp = np.array(tar_points[3]) - np.array(tar_points[2])
        bw_tp = np.array(tar_points[1]) - np.array(tar_points[0])
        up_op = np.array(self.points[3]) - np.array(self.points[2])
        ...
        inc_step = norm(inc_grid)
        self.logger.info('THE INITIAL INCREASING STEP LENGTH: %.4e', inc_step)
        curr_params = init_params
        self.logger.info('CURRENT PARAMS OF THE ITERATION: %s', repr(curr_params))

        def iteration(step):
            next_params = curr_params + step * slopes
        ...
        while err > self.error and satisfied:
            ...
            # the errors of the next try
            next_err = iteration(inc_step)
            if next_err > err:
                self.logger.warn(f'NEWERROE {new_err:.4e}, NEW ERROR BIGGER THAN PREVIOUS ERROR')
                while next_err > err:
                    inc_step = inc_step/2
                    self.logger.debug(f'MODIFY THE CURRENT INC_STEP {inc_step:.4e}')
                    next_err = iteration(inc_step)
            ...
            curr_params += inc_step*slopes
            self.logger.info(f'''
            NEXT INC_STEP OF THE ITERATION : {inc_step:.4e}
